...
class MarketObserverDashboard {
    ...
    async fetchConfig() {
        try {
            ...
            const response = await fetch(`${protocol}//${hostname}:8001/api/config`);
    ...
    startWebSocket() {
        ...
        this.ws.onmessage = (event) => {
            try {
                ...
                if (incoming.type === 'INITIAL') {
                    // Full snapshot replacement
                    this.data = incoming;
                    console.log('Dashboard data loaded (INITIAL)');
                    this.scheduleUpdate();
                } else if (incoming.type === 'UPDATE') {
                    // Partial merge
                    this.mergeData(incoming);
                    this.scheduleUpdate();
                } else {
                    // Fallback for legacy
                    if (!incoming.type) {
    ...
    getDisplayData() {
        ...
        symbols.forEach(sym => {
            ...
            if (aggregations[sym] && aggregations[sym][this.currentTimeframe]) {
                ...
                let agg = aggregations[sym][this.currentTimeframe];

                // Handle array (take last candle) or single object
                if (Array.isArray(agg)) {
                    agg = agg.length > 0 ? agg[agg.length - 1] : {};
                }
    ...
    updateAnalysisBlocks(data) {
        ...
        const sortedByCorr = [...dataArray].sort((a, b) => (b.price_volume_correlation || 0) - (a.price_volume_correlation || 0));
        const topCorr = sortedByCorr.slice(0, 5);

        this.renderList('correlationList', topCorr, (item) => {
            const val = item.price_volume_correlation || 0;
            const color = val > 0.5 ? 'text-success' : (val < -0.5 ? 'text-danger' : 'text-muted');
            return `<div class="list-group-item list-group-item-action d-flex justify-content-between align-items-center px-3 py-2 border-0 bg-transparent">
                <span class="item-symbol fw-bold text-truncate" title="${item.symbol}" style="max-width: 60%;">${item.symbol}</span>
                <span class="item-value ${color} text-nowrap ml-2">${val.toFixed(2)}</span>
            </div>`;
        });
    }
